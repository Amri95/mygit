...
def hidden_layer(input_sample):
    kernel_size = vector_size/8
    pixel_features = np.empty([kernel_size,8])
    for k in range(0, kernel_size):
        pixel_features[k,0:8]= input_sample[k:vector_size:kernel_size]
...
def run():
    ...
    Gw_l1= zeros((kernel_size, 8, hidden_neuron_num))
    ...
    for Epoch in range(0,Epoch_N):
        ...
        for Batch in range(0,Sample_num/Batch_num):
            ...
            for n in range(Batch*Batch_num,(Batch+1)*Batch_num):
                ...
                pixel_features=hidden_layer(input_sample)
                
                input_sample_signal = pixel_features + (pixel_features == 0)*(-1)
                input_layer2 = -1/hidden_neuron_num*np.ones((kernel_size, 8, hidden_neuron_num))
#                output_hidden_signal = np.zeros((8,1))
                # training in first layer
                for k in range(0, kernel_size) : 
                    output_hidden_signal = np.dot(input_sample_signal[k],Gw_l1[k])
                    sort_temp = np.argsort(-output_hidden_signal,0)
                    Gw_l1[k,:,sort_temp[0]] += pixel_features[k]
#                    input_layer2[k] = -1/hidden_neuron_num*ones((8,hidden_neuron_num))
                    input_layer2[k,sort_temp[0]] = 1
